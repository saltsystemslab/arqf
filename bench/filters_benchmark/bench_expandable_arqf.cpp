...
inline QF* init_qf(const t_itr begin, const t_itr end, const double bpk, Args... args)
{
  ...
  qf_malloc(qf, n_slots, key_size, memento_bits, QF_HASH_DEFAULT, seed, true);
  ...
  std::sort(key_hashes.begin(), key_hashes.end(), [=](const auto& lhs, const auto& rhs) { return qf_hash_cmp(qf, lhs, rhs) < 0; });
