...
struct zipfian {
    ...
    uint64_t N;                      // N, the size of the universe.
    double H_Ns;                 // H_{N,s}.
...
ZIPFIAN create_zipfian (double s, uint64_t N, long int (*randomfun)(void)) {
    assert(s > 0);
    assert(0 < N);
...
void generate_random_keys (uint64_t *elems, uint64_t N, long gencount, double s) {
	int i;
	uint32_t *counts;
	/*struct timeval a,b,c;*/
	printf("Generating %ld elements in universe of %lu items with characteristic exponent %f\n",
				 gencount, N, s);
	/*gettimeofday(&a, NULL);*/
